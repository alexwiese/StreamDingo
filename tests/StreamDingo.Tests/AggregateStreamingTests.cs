...
public class AggregateStreamingTests
{
    ...
    public async Task AggregateStreams_ShouldBeIndependentAndPartitioned()
    {
        ...
        string user1StreamId = $"user-{user1Id}";
        string user2StreamId = $"user-{user2Id}";
        ...
        long user1StreamVersion = await eventStore.GetStreamVersionAsync(user1StreamId);
        long user2StreamVersion = await eventStore.GetStreamVersionAsync(user2StreamId);
