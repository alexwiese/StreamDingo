...
public interface ISnapshotStore<TSnapshot> where TSnapshot : class
{
    ...
    Task SaveSnapshotAsync(string streamId, Snapshot<TSnapshot> snapshot, CancellationToken cancellationToken = default);
    ...
    Task<Snapshot<TSnapshot>?> GetLatestSnapshotAsync(string streamId, CancellationToken cancellationToken = default);
    ...
    Task<Snapshot<TSnapshot>?> GetSnapshotAtVersionAsync(string streamId, long maxVersion, CancellationToken cancellationToken = default);
    ...
    /// <param name="cancellationToken">Cancellation token.</param>
    /// <returns>A task representing the asynchronous operation.</returns>
    Task DeleteSnapshotsAsync(string streamId, CancellationToken cancellationToken = default);
}